...
% Vectorized moving window variance calculation
window = ones(1, 2 * win_sz + 1) / (2 * win_sz + 1);
mean_data = conv(data, window, 'same');
squared_data = conv(data .^ 2, window, 'same');
var_data = squared_data - mean_data .^ 2;

% Corrections based on threshold (similar to the original loop)
var_data(var_data < thresh) = 0;
